...
struct battery_config {
	int resolution;			// the resolution used to sample the ADC (12, 14, 16 or 18 bits, use the defines from the ADC)
...
int battery_get_charge(struct battery_charge* arg);				// read the global charge
int battery_get_config(struct battery_config* cfg);				// read the global config
int battery_set_config(struct battery_config* cfg);				// write the global config
